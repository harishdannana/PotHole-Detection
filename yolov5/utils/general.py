...
ROOT = FILE.parents[2]  # Project root directory (assuming yolov5 is a subdirectory)
...
def check_dataset(data_path, autodownload=True):
    """Validates and/or auto-downloads a dataset, returning its configuration as a dictionary."""
    # Download (optional)
    extract_dir = ""
    if isinstance(data_path, (str, Path)) and (is_zipfile(data_path) or is_tarfile(data_path)):
        download(data_path, dir=f"{DATASETS_DIR}/{Path(data_path).stem}", unzip=True, delete=False, curl=False, threads=1)
        data_path = next((DATASETS_DIR / Path(data_path).stem).rglob("*.yaml"))
        extract_dir, autodownload = data_path.parent, False

    # Read yaml (optional)
    if isinstance(data_path, (str, Path)):
        data = yaml_load(data_path)  # dictionary
    else:  # data_path is already a dictionary from previous call
        data = data_path
    ...
    # Resolve paths
    # Use the parent of data_path if data.get("path") is not explicitly set
    path = Path(extract_dir or data.get("path") or Path(data_path).parent)
